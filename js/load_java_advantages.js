document.addEventListener("DOMContentLoaded", () => {
    fetch("../data/java_advantages.xml")
        .then(r => r.text())
        .then(xmlText => {
            const xml = new DOMParser().parseFromString(xmlText, "text/xml");
            const modules = xml.getElementsByTagName("Module");
            const container = document.getElementById("java-advantages");
            ...
            Array.from(modules).forEach((mod, index) => {
                ...
                card.innerHTML = `
                    <div class="card h-100 bg-dark text-white text-center p-3 java-card">
                        <div class="card-body">
                ...
                // Плавное появление
                setTimeout(() => {
                    card.querySelector(".java-card").classList.add("fade-in");
                }, index * 150);
